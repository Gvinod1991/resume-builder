...
export const TagsInput = ({ tagLabel, selectedTags }: TagsInput) => {
  ...
  return (
    <div>
      {tags.map((tag, index) => (
        <li key={index} className="shadow-sm bg-indigo-600 p-2 m-1 inline-block text-xl text-white mb-1">
          <span>{tag}</span>
          <XIcon className="inline h-6 w-5 text-white cursor-pointer" onClick={() => removeTags(index)} />
        </li>
      ))}
      <Input
